...
const POSInterface = () => {
  ...
  const [viewCategories, setViewCategories] = useState(true); // To toggle between categories and items
  ...
  // Keydown event handler for navigating categories/items and pressing Esc
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === "ArrowDown" || e.key === "ArrowUp") {
      // Navigate through categories and items
      const activeCategoryIndex = categoryRefs.current.findIndex(
        (ref) => ref === document.activeElement
      );
      const activeItemIndex = itemRefs.current.findIndex(
        (ref) => ref === document.activeElement
      );

      if (activeCategoryIndex >= 0 && viewCategories) {
        const nextCategoryIndex =
          activeCategoryIndex + (e.key === "ArrowDown" ? 1 : -1);
        if (nextCategoryIndex >= 0 && nextCategoryIndex < dummyCategories.length) {
          categoryRefs.current[nextCategoryIndex]?.focus();
        }
      }

      if (activeItemIndex >= 0 && !viewCategories) {
        const nextItemIndex =
          activeItemIndex + (e.key === "ArrowDown" ? 1 : -1);
        if (nextItemIndex >= 0 && nextItemIndex < filteredItems.length) {
    ...
    if (e.key === "Enter") {
      if (viewCategories) {
        ...
        if (activeCategoryIndex >= 0) {
          setSelectedCategory(dummyCategories[activeCategoryIndex].name);
          setViewCategories(false); // Show items after selecting a category
        }
      } else {
        const activeItemIndex = itemRefs.current.findIndex(
          (ref) => ref === document.activeElement
        ...
        if (activeItemIndex >= 0) {
          addItemToBill(filteredItems[activeItemIndex]);
        }
      }
    }
    ...
    if (e.key === "Escape") {
      if (!viewCategories) {
        setViewCategories(true); // Go back to categories when Esc is pressed
      }
    }
  };
        ...
        {viewCategories ? (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mb-6">
            {dummyCategories.map((category, index) => (
              <Card
                key={category.id}
                className={`cursor-pointer p-4 text-center text-white bg-gradient-to-r ${category.color} hover:shadow-lg`}
                onClick={() => {
          ...
          <CardContent className="p-4 h-full flex flex-col">
            <div className="flex-1 overflow-y-auto space-y-2">
              ...
              {currentBill.length === 0 && (
                <div className="text-center text-muted-foreground py-8">No items in bill</div>
              )}
            </div>
